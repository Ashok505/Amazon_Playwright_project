# Test info

- Name: Download, edit C3 cell, upload, and clean up Excel file
- Location: /home/ashok/Desktop/amazon-playwright-js/tests/downloadEditUpload.spec.js:6:5

# Error details

```
Error: locator.fill: Target page, context or browser has been closed
Call log:
  - waiting for locator('#username')

    at /home/ashok/Desktop/amazon-playwright-js/tests/downloadEditUpload.spec.js:12:35
```

# Test source

```ts
   1 | import { test, expect } from '@playwright/test';
   2 | import xlsx from 'xlsx';
   3 | import fs from 'fs';
   4 | import path from 'path';
   5 |
   6 | test('Download, edit C3 cell, upload, and clean up Excel file', async ({ page }) => {
   7 |   const downloadsDir = path.resolve(__dirname, '../downloads');
   8 |   if (!fs.existsSync(downloadsDir)) fs.mkdirSync(downloadsDir);
   9 |
  10 |   // Step 1: Login
  11 |   await page.goto('https://hcm-auth-uat.teamlease.com:3035/realms/hiretech/protocol/openid-connect/auth?...');
> 12 |   await page.locator('#username').fill('T10533');
     |                                   ^ Error: locator.fill: Target page, context or browser has been closed
  13 |   await page.getByRole('button', { name: 'Continue' }).click();
  14 |   await page.locator('#password').fill('12345');
  15 |   await page.getByRole('button', { name: 'Login' }).click();
  16 |
  17 |   // Step 2: Trigger download
  18 |   await page.getByRole('button', { name: 'Client' }).click();
  19 |   await page.getByRole('textbox', { name: 'Search Client / ID' }).fill('afbch');
  20 |   await page.getByRole('option', { name: 'AGS CINEMAS PRIVATE LIMITED -' }).click();
  21 |   await page.getByRole('img', { name: 'Download master data' }).click();
  22 |   await page.getByText('Download Client MOL Edit Data').click();
  23 |   await page.getByRole('checkbox', { name: 'Name as Aadhar' }).check();
  24 |   await page.getByRole('checkbox', { name: 'Aadhar No' }).check();
  25 |
  26 |   const [download] = await Promise.all([
  27 |     page.waitForEvent('download'),
  28 |     page.getByRole('button', { name: 'Download XLSX' }).click()
  29 |   ]);
  30 |
  31 |   const originalFileName = await download.suggestedFilename();
  32 |   const downloadedFilePath = path.join(downloadsDir, originalFileName);
  33 |   await download.saveAs(downloadedFilePath);
  34 |
  35 |   // Step 3: Edit Excel file (C3 cell)
  36 |   const workbook = xlsx.readFile(downloadedFilePath);
  37 |   const sheet = workbook.Sheets[workbook.SheetNames[0]];
  38 |   sheet['C3'] = { t: 's', v: 'New Name Automated' };
  39 |   const editedFilePath = path.join(downloadsDir, 'edited_file.xlsx');
  40 |   xlsx.writeFile(workbook, editedFilePath);
  41 |
  42 |   // Step 4: Upload the edited file
  43 |   await page.getByRole('img', { name: 'Bulk Upload' }).click();
  44 |   await page.getByText('Client MOL Edit Data').click();
  45 |   const fileInput = await page.locator('input[type="file"]');
  46 |   await fileInput.setInputFiles(editedFilePath);
  47 |   await page.getByTestId('upload-button').click();
  48 |
  49 |   // Step 5: Assert upload success (optional)
  50 |   // await expect(page.getByText('Upload successful')).toBeVisible();
  51 |
  52 |   // Step 6: Delete files
  53 |   fs.unlinkSync(downloadedFilePath);
  54 |   fs.unlinkSync(editedFilePath);
  55 | });
  56 |
```